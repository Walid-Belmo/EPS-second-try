...
## SECTION G: SOURCE PDS FILE AND MODULE ORGANIZATION

These rules apply specifically to `src-pds`.

The goal is that a new student can understand the project structure from the
folder names and file names before opening any file.

### Rule G1: File Names Describe A Set Of Functions, Not One Function Call

A `.c` file is a module.
Its name describes what the functions inside the file make possible.

A function name is a verb phrase:

```c
run_power_state_machine_with_injected_sensor_values();
```

A file name describes the group of functions that support that capability:
...
The file name must answer:

```text
What capability does this file provide?
```

It must not merely label the topic area.

### Rule G2: Folders Describe Layers Of Responsibility

`src-pds` must not become one flat folder containing every file.

Files are grouped into folders by responsibility.
The folder name explains the layer.
The file name explains the capability inside that layer.
...
src-pds/
  ...
  board_hardware_startup/
    functions_to_initialize_board_hardware_before_main_loop_runs.c

  communication_with_esp32/
    functions_to_read_chips_commands_received_from_esp32.c
    functions_to_execute_board_commands_received_from_esp32.c
    functions_to_send_chips_replies_to_esp32.c
  ...
  status_reporting_to_esp32/
    functions_to_build_status_replies_sent_to_esp32.c
    functions_to_stream_status_replies_to_esp32.c

  board_outputs/
    functions_to_request_pwm_output_before_safety_checks.c
    functions_to_apply_allowed_outputs_to_board_hardware.c

  shared_helpers/
    functions_to_read_and_write_little_endian_values.c
```

### Rule G3: Dependencies Go Downward, Not Sideways

A module may call a smaller, lower-level module.
A module must not casually call a parallel module just because both are useful.

This keeps the code understandable.
The reader should be able to say:

```text
High-level files coordinate behavior.
Lower-level files provide small tools.
```
...
```text
mppt test code -> status streaming code
status streaming code -> command execution code
state transition code -> command parser code
```

Those modules are adjacent in usage.
They should communicate through lower-level stored values or through an explicit
top-level coordinator, not by calling each other directly.

### Rule G4: Shared RAM Is A Lower-Level Service

Values changed by ESP32 commands live in one lower-level module.

Other modules may read or update those values only through clear functions.
They should not all include a giant exposed struct and modify every field
directly.

Wrong:

```c
runtime_state->snapshot.panel_voltage_in_millivolts = panel_voltage;
runtime_state->requested_mode = PDS_REQUESTED_MODE_MPPT_TEST;
```

Better:

```c
store_requested_mode_as_mppt_test();
store_latest_panel_voltage_for_status(panel_voltage_in_millivolts);
```

The goal is not to hide information.
The goal is to make every state change obvious and searchable.

### Rule G5: Prefixes Are Not A Substitute For Structure

Do not add the same prefix to every file just because all files are in one
folder.
...
The file name should explain the file's role.
