 ...
 * Transmits log messages from a circular RAM buffer to SERCOM5 UART (PB22)
 ...
 * Peripheral: SERCOM5 (UART TX), DMAC channel 0, PORT group B
 * Pins: PB22 (TX, mux D → SERCOM5 PAD[2])
...
#define USER_LED_PIN_FOR_UART_TX                22u  /* PB22 */
...
static void configure_pb22_as_sercom5_uart_transmit_pin(void);
...
void debug_log_initialize_dma_uart_on_sercom5_pb22(void)
{
    ...
    configure_pb22_as_sercom5_uart_transmit_pin();
...
void debug_log_write_labeled_signed_integer(const char *label, int32_t value)
{
    if (value < 0)
    {
        ...
        debug_log_write_labeled_unsigned_integer("", absolute_value);
    }
    else
    {
        debug_log_write_labeled_unsigned_integer(label, (uint32_t)value);
    }
}
...
static void configure_pb22_as_sercom5_uart_transmit_pin(void)
{
    /* PB22 defaults to GPIO after reset. We need to route it to SERCOM5.
     *
     * Step 1: Enable the peripheral mux on PB22 (PMUXEN bit in PINCFG).
     *         Without this, the pin stays as GPIO regardless of PMUX setting.
     *
     * Step 2: Set the mux function to D (SERCOM-ALT).
     *         PB22 can reach two different SERCOMs depending on the mux:
     *           Mux C = SERCOM (primary)  → wrong SERCOM for this pin
     *           Mux D = SERCOM-ALT        → SERCOM5 PAD[2] (TX)
     *         Using the wrong mux routes PB22 to a different SERCOM.
     *
     * PORT_PMUX register: PB22 is pin 22. PMUX index = 22/2 = 11.
     * PB22 is an even pin, so we write to the PMUXE (even) nibble (bits 3:0). */
    PORT_REGS->GROUP[1].PORT_PINCFG[22] |= PORT_PINCFG_PMUXEN_Msk;

    uint8_t current_pmux_value = PORT_REGS->GROUP[1].PORT_PMUX[11];
    ...
    PORT_REGS->GROUP[1].PORT_PMUX[11] = current_pmux_value;
}
...
static void configure_sercom5_as_uart_transmitter_at_115200_baud(void)
{
    ...
    SERCOM5_REGS->USART_INT.SERCOM_CTRLA =
        ...
        SERCOM_USART_INT_CTRLA_TXPO(1u);             /* TXPO=1: TX on PAD[2] = PB22 */
